...
int Set_I_Charge (int level)
{
	...
	system("chmod +rw /sys/class/power_supply/ltc4015-charger/constant_charge_current");
	usleep(200000);

	fd=open("/sys/class/power_supply/ltc4015-charger/constant_charge_current", O_WRONLY); // must be change
	if (fd==-1)
	{
	...
	if ((level>=1)&&(level<=32))
	{
		cnt_byte=snprintf(buf, sizeof(buf), "%i", level);
		if (write(fd, buf, cnt_byte)!=cnt_byte)
		{
